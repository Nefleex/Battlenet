  ...
  try {
    if (status === 0) {
      ...
      if (limit) {
        const latestTimestamp = await getLatestTimestamp();
        const items = await db.sequelize.query(
          `SELECT a.itemId, a.buyout, i.name FROM StandBys AS a, Items AS i WHERE batchTimeId="${latestTimestamp}" and a.itemId=i.id ORDER BY buyout DESC LIMIT ${limit}`,
          {
            type: db.sequelize.QueryTypes.SELECT
          }
        );
        ...
        const latestTimestamp = await getLatestTimestamp();
        const items = await db.sequelize.query(
          `SELECT a.itemId, a.buyout, i.name FROM StandBys AS a, Items AS i WHERE batchTimeId="${latestTimestamp}"  ORDER BY buyout DESC`,
          {
            type: db.sequelize.QueryTypes.SELECT
          }
        );
